...
function initOptiBot() {
    ...
    // Configurar cámara - ajustada para mejor vista del robot
    const aspect = container.clientWidth / container.clientHeight;
    camera = new THREE.PerspectiveCamera(45, aspect, 0.1, 1000);
    camera.position.set(0, 2, 15); // Elevé un poco la cámara
    camera.lookAt(0, 0, 0);
...
function createRobot() {
    // Crear grupo principal del robot
    robot = new THREE.Group();
    
    // Crear un contenedor intermedio para aplicar las rotaciones correctamente
    const robotContainer = new THREE.Group();
    
    // Base flotante (ahora será la parte inferior)
    const baseGeometry = new THREE.CylinderGeometry(1.8, 1.5, 0.3, 32);
    ...
    base.rotation.x = Math.PI / 2; // Rotar la base para que esté horizontal
    ...
    body.position.y = -1;
    body.position.z = 0; // Asegurar que esté centrado
    ...
    headGroup.position.z = 0; // Asegurar que esté centrado
    ...
    // Ojos - ajustados para mirar hacia adelante
    ...
    robotContainer.add(leftArmGroup);
    ...
    robotContainer.add(rightArmGroup);
    
    // Agregar el contenedor al robot principal
    robot.add(robotContainer);
    
    // CORRECCIÓN PRINCIPAL: Aplicar las rotaciones necesarias
    // 1. Primero enderezar el robot (como levantar una ficha de dominó)
    robotContainer.rotation.x = -Math.PI / 2;
    
    // 2. Luego girar 180 grados para que mire de frente
    robotContainer.rotation.z = Math.PI;
    
    // Ajustar la posición del robot para que esté bien centrado
    robot.position.y = 2; // Elevar el robot para que no esté hundido
...
function animate() {
    ...
    robot.position.y = 2 + Math.sin(Date.now() * 0.001) * 0.2;
